...
def CFD_inputs_from_line(intercept_coordinates):
    ...
    for i in range(len(intercept_coordinates)):
        ...
        pwk_results.append( PWK_CFD_inputs_from_intersection(H0,P0,P_arc=240*10**3,R_jet = 0.045, R_model = 0.0381))
...
def compare_beta(resolution):
    beta_square_PWK, P0_grid_PWK, H0_grid_PWK, mdot_linspace, P0_linspace, H_add_linspace = beta_curve_PWK(mdot_range=[0.005,0.05], P_arc = 240*10**3, P0_range=[50000,500*10**3], R_jet = 0.045, R_model = 0.0381, resolution=resolution)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # beta_square_PWK, P0_grid_PWK, H0_grid_PWK, mdot_linspace, P0_linspace, H_add_linspace = beta_curve_PWK(mdot_range=[0.005,0.05], P_arc = 240*10**3, P0_range=[10000,500*10**3], R_jet = 0.03, R_model = 0.0381, resolution=20)
        # ax = plt.axes(projection='3d')
        # surf = ax.plot_surface(H0_grid_PWK/(10**6), P0_grid_PWK/(10**3), beta_square_PWK/(10**3), cmap=cm.coolwarm, linewidth=1, antialiased=False, rcount=200, ccount=200)
        # plt.show()
        #interpolation_of_beta_curve(beta_square_PWK, H0_grid_PWK, P0_grid_PWK, beta_square_flight, H0_grid_flight, P0_grid_flight)
        
        # beta_square_PWK, P0_grid_PWK, H0_grid_PWK, mdot_linspace, P0_linspace, H_add_linspace = beta_curve_PWK(mdot_range=[0.005,0.05], P_arc = 240*10**3, P0_range=[600,500*10**3], R_jet = 0.03, R_model = 0.0381, resolution=10)
